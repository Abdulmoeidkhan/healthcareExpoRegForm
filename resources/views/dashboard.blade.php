...
<head>
    <meta charset="UTF-8">
    <title>HealthCare Expo - Users Dashboard</title>
...
<body>
    ...
    <div class="dashboard">
        @if($errors->any())
        <script>
            alert('CNIC , CONTACT or EMAIL address is already registered with us')
        </script>
        @endif
        <aside class="search-wrap">
            ...
            <div class="user-actions">
                ...
                <a href="{{ route('slipDashboard') }}" style="
                text-decoration:none;
                border: 0;
                background: none;
                width: 32px;
                ...
                <section class="info-boxes">
                    <div class="info-box active">
                        ...
                        <div class="box-content">
                            <span class="big"><?php echo $registrationsLength; ?></span>
                            Forms Submitted
                        </div>
                    </div>
                </section>
                    ...
                    <table id="table" data-toggle="table" data-search="true" data-filter-control="true" data-show-export="true" data-show-refresh="true" data-show-toggle="true" data-pagination="true" data-toolbar="#toolbar" class="table-responsive" data-escape="false">
                        <thead>
                            <tr>
                                ...
                                <th data-field="CellPhone" data-filter-control="input" data-sortable="true">Cell Phone</th>
                                <th data-field="NationalID" data-filter-control="input" data-sortable="true">National ID</th>
                                ...
                                <th data-field="actions" data-sortable="false" data-force-hide="true">Slip Generation</th>
                        ...
                        <tbody id="table-body">
                            <?php foreach ($registrations as $key => $registration) { ?>
                                <tr id="{{$registration['personsId']}}">
                                    <td class="bs-checkbox">
                                        <input data-index="{{$key+1}}" name="btSelectItem" type="checkbox">
                                    </td>
                                    <td class="tableexport-string">{{$registration->Name}}</td>
                                    <td class="tableexport-string">{{$registration->Email}}</td>
                                    <td class="tableexport-string">{{$registration->CellPhone}}</td>
                                    <td class="tableexport-string">{{$registration->NationalId}}</td>
                                    <td class="tableexport-string">{{$registration->PersonsId}}</td>
                                    <td class="tableexport-string">{{$registration->Badge}}</td>
                                    <td class="tableexport-string">{{$registration->Nationality}}</td>
                                    <td>
                                        <div class="text-center">
                                            @if($user === 0)
                                                @if($registration->slipGenerated < 1)
                                                <form action="/slipGenerator" method="POST">
                                                    @csrf
                                                    <select name="numberOfslips" id="numberOfslips" value='{{$registration->slips}}'>
                                                        @for($i=0;$i < 11; $i++) @if($registration->slips === $i)
                                                    ...
                                                    <input type="hidden" value="{{$registration->Name}}" name="registrantName" />
                                                    <input type="hidden" value="{{$registration->PersonsId}}" name="registrantId" />
                                                    <input type="hidden" value="{{$registration->Badge}}" name="registrantBadge" />
                                                    <input type="submit" class="btn btn-primary" value="Create" name="submitSlip" />
                                                </form>
                                                @else
                                                    <a class="btn btn-success" target="_blank" href="{{'http://'.$host.'/slipDashboard?Badge='.$registration->Badge}}">View</a>
                                                @endif
                                            @else
                                                @if($registration->slipGenerated < 1)
                                                    <select name="numberOfslips" id="numberOfslips" value='{{$registration->slips}}'>
                                                        @for($i=0;$i < 11; $i++)
                                                            @if($registration->slips === $i)
                                                                <option value="{{$i}}" selected>{{$i}}</option>
                                                            @else
                                                                <option value="{{$i}}">{{$i}}</option>
                                                            @endif
                                    ...
                                    <!-- <a target="_blank" class="btn btn-primary" href="{{'http://'.$host.'/completeProfile/'.$registration['personsId']}}">View Profile</a>
                                        <a target="_blank" class="btn btn-warning" href="{{'http://'.$host.'/badge/'.$registration['personsId']}}">View Badge</a> -->
                                    <!-- </td> -->
