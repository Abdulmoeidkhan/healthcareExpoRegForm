...
class slipValidityChangeController extends Controller
{
    public function slipValidityChange(Request $req, $badge, $SlipCode, $status)
    {
        if (Auth::check()) {
            $updatedSlip = slip::where('SlipCode', $SlipCode)->update(['used' => $status,'updated_by'=>Auth::id()]);
